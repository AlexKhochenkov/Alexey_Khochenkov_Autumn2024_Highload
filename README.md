...
В среднем каждый день создаётся 50 млрд событий [[1]](https://earthweb.com/blog/google-calendar-users/). Поделив это число на DAU, получаем 50000 млн / 250 млн = 200 событий на одного пользователя ежедневно.

События хранятся неограниченное количество времени, если не удалять их вручную, однако есть лимит в 10000 событий, после которого старые события удаляются [[6]](https://classroom.synonym.com/google-calendar-limits-17378.html). Так что возьмём, что на каждого пользователя необходимо мечто под 10000 событий.
...
Параметр          | Вес
-------------| -------------
Добавление событий                         | 200 запросов/день
