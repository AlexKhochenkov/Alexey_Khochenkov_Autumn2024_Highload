...
| User                 | Calendar             | Event                 | Reminder       |
| -------------------- | -------------------- | --------------------- | -------------- |
| id                   | id                   | id                    | id             |

Шардировать рационально по полю id, чтобы обеспечить равномерное распределение по шардам.

Сессию шардировать не будем, так как она не имеет большого размера и в этом нет необходимости.
...
Для репликации во всех таблицах разумно использовать схему с одним master для записи и многими slave для чтения, так как в нашем приложении основная нагрузка приходится именно на чтение.
