...
Для всех случаев, где от индекса нужно точное сопадение, например, id, будем использовать hash-индексы, так как они наиболее эффективны для таких задач. Во всех остальных случаях будем использовать B-tree, так как для этих задач функциональность hash индексов не подходит.

В качестве индексов выбраны:

* Для таблицы Calendar индекс на user_id(hash), для быстрой выдачи календарей пользователя. Также необходим индекс на unique_id(hash) для быстрого изменения и удаления всех копий одного календаря.

* Для таблицы Event индекс на user_id(hash) и creator_id(hash), для быстрого отображения событий пользователя, индекс на start_date(B-tree) для отображения событий в нужном временном диапозоне, а также индекс на name(B-tree) для быстрого поиска событий. Также необходим индекс на unique_id(hash) для быстрого изменения и удаления всех копий одного календаря. Ещё необходим индекс на calendar_id(hash) для отображения всех событий в выбранном календаре.

* Для таблицы Reminder индекс на remind_date(B-tree), так как необходимо быстро отображать напоминания при достижении временной отметки и индекс на user_id(hash) для отображения напоминаний пользователя.

* Для таблицы Elastic_Search индекс на email(B-tree) и username(B-tree), для быстрого поиска пользователей.
...
| Cassandra     | 5 427 * 3 + 11 303 = 27 584 | 276 | 15 * 8 Б * 500 млн = 55 Gb. Возьмём 60 Gb. | 8 159 Gb + 47 Gb + 1 805 Gb * 20 + 44 Tb * 20 = 929 Tb |
...
| Сервис        |  Конфигурация                                 | Ядра  | Количество на 1 ДЦ    | Цена за 1 | Аренда за месяц    |
|---------------|-----------------------------------------------|-------|--------------------   |---------  |-------    |
| Kube node     | 1U 8xAMD EPYC 9124P 16x32GB 1xNVMe1T          | 1x128  | 17                   | €20 775   | €2430      |
| Tarantool     | 1U AMD EPYC 7543P 32GB DDR4 x6 Samsung NVME 1ТБ| 1x32 | 15                    | €5 248    | €880       |
| ElasticSearch | 1U AMD EPYC 7543P 32GB RAM x2 NVMe 8 ТБ NVME  | 1x16  | 5                     | €6 032    | €1010      |
| Cassandra     | 1U AMD EPYC 7402P 8GB RAM  x2 16ТБ NVME       | 1х24  | 17                    | €3 934    | €650       |
