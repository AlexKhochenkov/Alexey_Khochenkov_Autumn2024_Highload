...
Для таблиц Reminder и Event рационально использовать Cassandra, так как мы имеем большие нагрузки на чтение и запись, для чего подходит данная СУБД.

Для таблиц Calendar и User рационально использовать PostgreSQL, так как эти данные нужны в основном для чтение и редко для записи, для чего подходит PostgreSQL.

Для хранения сессий рационально использовать in-memory СУБД, например Tarantool.

| User                 | Calendar             | Event                 | Session          | Reminder       |
| -------------------- | -------------------- | --------------------- | ---------------- | -------------- |
| PostgreSQL           | PostgreSQL           | Cassandra             | Tarantool        | Cassandra      |
...
Приглашения должны быть доставлены до пользователя, однако может возникнуть проблема, при которой ошибки отправки будут вызывать задержки в очереди отправки приглашений, что может негативно сказаться на работоспособности системы. Для решения этой проблемы можно создать в БД таблицу ошибок отправки, в которую будут помещаться запросы, недошедшие до пользователя. Таким образом можно разгурзить очередь от ошибочных запросов, а сами запросы повторно отправлять через Kafka.
