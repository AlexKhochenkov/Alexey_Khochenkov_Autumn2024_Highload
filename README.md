...
* Для таблицы User индекс на username, для поиска пользователей, которых нужно прикрепить к событию

* Для таблицы Calendar индекс на user_id, для быстрой выдачи календарей пользователя

* Для таблицы Event индекс на user_id, для быстрого отображения событий пользователя, индекс на start_date для отображения событий в нужном временном диапозоне, а также индекс на name для быстрого поиска событий.
...
Чтобы избавиться от таблиц User_to_calendar и User_to_event, так как они будут требовать JOIN запросов, что крайне не эффективно, добавим в таблицы Event и Calendar поля-массивы users, которые будут содержать id всех участников события и календаря соответсвенно, а в User добавим поля foreign_events и foreign_calendars, которые будут содержать id календарей и событий, к которым прикреплён пользователь.
...
Для таблиц User, Reminder и Event рационально использовать Cassandra, так как мы имеем большие нагрузки на чтение и запись, для чего подходит данная СУБД.

Для таблицы Calendar рационально использовать PostgreSQL, так как эти данные нужны в основном для чтение и редко для записи, для чего подходит PostgreSQL.

Для хранения сессий рационально использовать in-memory СУБД, например Tarantool.

| User                 | Calendar             | Event                 | Session          | Reminder       |
| -------------------- | -------------------- | --------------------- | ---------------- | -------------- |
| Cassandra            | PostgreSQL           | Cassandra             | Tarantool        | Cassandra      |
...
Для репликации разумно использовать схему с одним master для записи и многими slave для чтения, так как в нашем приложении основная нагрузка приходится именно на чтение.
...
| PostgreSQL     | Дополнительная база данных               | Реляционная СУБД, обеспечивающая высокую надежность, масштабируемость и поддержку сложных запросов и транзакций. Подходит для хранения и обработки информации, которая не находиться под существенной нагрузкой, особенно если эта нагрузка приходится на чтение (например, таблица календарей). |
...
| S3             | Хранилище файлов и резервное копирование | Облачное хранилище объектов, предоставляющее безопасное, масштабируемое и доступное решение для хранения и управления данными любого объема. Используется для хранения аватарок пользователя, так как подходит для хранения фотографий большого размера. |
